...
```{bash}
cat "my_dataset.tsv" | head -n1 | cut -f 1,2,5 > "my_dataset_new.tsv" 
cat "my_dataset.tsv" | grep "setosa" | cut -f 2,3,6  | sort -rk1 >> "my_dataset_new.tsv"
...
mkdir my_dir
mv my_dir my_dir2
touch my_dir2/my_file.txt  
outfile="my_dir2/my_file.txt"
...
```{bash}
infile="grafimo_out.gff"
fwdfile="grafimo_out_fwd.gff"
revfile="grafimo_out_rev.gff"

cat $infile | head -n1 > $fwdfile
awk '$7 == "+"' $infile >> $fwdfile

cat $infile | head -n1 > $revfile
awk '$7 == "-"' $infile >> $revfile
```
...
function process_gff {

  feature_num=`wc -l $1`
  feature_num=$(($feature_num - 1))
  echo "The total number of feature in ${1} is ${feature_num}"
  
  fwd_features=`awk '$7 == "+"' $1 | wc -l`
  echo "The number of features belonging to the forward strand are ${fwd_features}"
  
  rev_features=`awk '$7 == "-"' $gff | wc -l`
  echo "The number of features belonging to the reverse strand are ${rev_strand}"
  
  chroms=`awk '{print $1}' | sort -u`
  
  # number of features by chromosome
  for c in $chroms; do
    features_curr=`awk '$1 == "$c"' $1 | wc -l`
    echo "Chromosome ${c} has ${feature_curr} features"
  done
  
  # average score by chromsome
  for c in $chroms; do
    features_curr=`awk '$1 == "$c"' $1 | wc -l`
    avg_score=$((`awk '$1 == "$c"' $1 | awk '{sum+=$6}END{print sum}'` / $features_curr))
    echo "Chromosome ${c} has ${avg_score} average score"
  done
