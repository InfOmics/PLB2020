...
```
a) sort the rows in asceding order according to values in column 4
b) sort the rows in descending order according to values in column 4 (X)
c) sort the rows in descending order according to values in column 5
d) print the fourth column sorted 
e) sort in ascending order (according to their size) all the files in the current directory, which have '3' in their names
